...
export class ImShipmentAnalyticsCostRepository extends Repository {
    ...
    async getYtdShipmentByOrigin(whereObj: any, attributes?: any, transaction?: Transaction): Promise<BvYtdshipmentsDrilldown[]> {
        whereObj = (whereObj == undefined && whereObj == null) ? {} : whereObj
        return await new Promise((resolve, reject) => {
            BvYtdshipmentsDrilldown.findAll({
                where: whereObj, attributes: attributes
            }).then((get: BvYtdshipmentsDrilldown[]) => {
                resolve(get);
            }).catch((error: any) => {
                this.logger.error(error);
                reject(error);
    ...
    async getRegionalCostAnalysisTable(whereObj: any, flag: any): Promise<BvAggCostanalysis[]> {
        whereObj = (whereObj == undefined && whereObj == null) ? {} : whereObj
        return await new Promise((resolve, reject) => {
            if (flag === 'nopayload') {
                BvAggCostanalysis.findAll({
                    where: whereObj, attributes: [['origin_region','label'],['origin_region','origin_region'],['AllRegions','destination_origin'],'shipment_total_currency','mode_of_transport',[Sequelize.fn('count', Sequelize.col('shipment_id')), 'noofshipments'], [Sequelize.fn('count', Sequelize.col('hawb')), 'hawb'], [Sequelize.fn('sum', Sequelize.col('number_of_pieces')), 'quantity'], [Sequelize.fn('ROUND', Sequelize.fn('sum', Sequelize.col('calc_total_price_with_rate'))), 'cost'], [Sequelize.literal('ROUND(sum(`unit_cost_total_price`)/IFNULL(count(`hawb`),1),2)'), 'ratesUsed']], group: ['origin_region','shipment_total_currency','mode_of_transport']
                }).then((get: BvAggCostanalysis[]) => {
                    resolve(get);
                }).catch((error: any) => {
                    this.logger.error(error);
                ...
                BvAggCostanalysis.findAll({
                    where: whereObj, attributes: ['origin_region',['destination_origin','label'],['destination_origin','destination_origin'],'shipment_total_currency','mode_of_transport',[Sequelize.fn('count', Sequelize.col('shipment_id')), 'noofshipments'], [Sequelize.fn('count', Sequelize.col('hawb')), 'hawb'], [Sequelize.fn('sum', Sequelize.col('number_of_pieces')), 'quantity'], [Sequelize.fn('ROUND', Sequelize.fn('sum', Sequelize.col('calc_total_price_with_rate'))), 'cost'], [Sequelize.literal('ROUND(sum(`unit_cost_total_price`)/IFNULL(count(`hawb`),1),2)'), 'ratesUsed']], group: ['origin_region','destination_origin','shipment_total_currency','mode_of_transport']
                }).then((get: BvAggCostanalysis[]) => {
                    resolve(get);
                }).catch((error: any) => {
                    this.logger.error(error);
